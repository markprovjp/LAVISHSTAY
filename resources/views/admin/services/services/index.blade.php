<x-app-layout>
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        <!-- Page header -->
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            <div class="mb-4 sm:mb-0">
                <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold">Services Management</h1>
                <p class="text-sm text-gray-600 dark:text-gray-400">Manage all services and their configurations</p>
            </div>
            <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
                <a href="{{ route('admin.services.services.create') }}">
                    <button
                        class="btn bg-gray-900 text-gray-100 hover:bg-gray-800 dark:bg-gray-100 dark:text-gray-800 dark:hover:bg-white">
                        <svg class="fill-current shrink-0 xs:hidden" width="16" height="16" viewBox="0 0 16 16">
                            <path
                                d="M15 7H9V1c0-.6-.4-1-1-1S7 .4 7 1v6H1c-.6 0-1 .4-1 1s.4 1 1 1h6v6c0 .6.4 1 1 1s1-.4 1-1V9h6c.6 0 1-.4 1-1s-.4-1-1-1z" />
                        </svg>
                        <span class="max-xs:sr-only">Add Service</span>
                    </button>
                </a>
            </div>
        </div>

        <x-slot name="header">
            <div class="flex justify-between items-center">
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                    {{ __('Quản lý Dịch vụ') }}
                </h2>
                <a href="{{ route('admin.services.services.create') }}"
                    class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                    Thêm Dịch vụ mới
                </a>
            </div>
        </x-slot>

        <div class="py-12">
            <div class="">
                <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        @if (session('success'))
                            <div id="success-message"
                                 class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mb-4">
                                {{ session('success') }}
                            </div>
                        @endif

                        <!-- Table -->
                        <div class="">
                            <table class="w-full table-auto">
                                <thead>
                                    <tr class="bg-gray-50 dark:bg-gray-700">
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">ID</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Tên</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Giá (VND)</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Đơn vị</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Trạng thái</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Ngày tạo</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Hành động</th>
                                    </tr>
                                </thead>
                                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                    @forelse($services as $service)
                                        <tr>
                                            <td class="px-6 py-4 text-sm">{{ $service->service_id }}</td>
                                            <td class="px-6 py-4 text-sm">{{ Str::limit($service->name, 50) }}</td>
                                            <td class="px-6 py-4 text-sm">{{ number_format($service->price_vnd, 0, ',', '.') }}</td>
                                            <td class="px-6 py-4 text-sm">{{ $service->unit ?? 'N/A' }}</td>
                                            <td class="px-6 py-4">
                                                <button onclick="toggleStatus({{ $service->service_id }})"
                                                    class="status-toggle px-2 inline-flex text-xs font-semibold rounded-full
                                                    {{ $service->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                                    {{ $service->is_active ? 'Hoạt động' : 'Không hoạt động' }}
                                                </button>
                                            </td>
                                            <td class="px-6 py-4 text-sm">{{ $service->created_at->format('d/m/Y H:i') }}</td>
                                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap text-center">
                                                <div class="relative inline-block text-left">
                                                    <button type="button"
                                                        class="button-action inline-flex items-center justify-center w-8 h-8 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-violet-500 transition-colors duration-200"
                                                        onclick="toggleDropdown({{ $service->service_id }})"
                                                        id="dropdown-button-{{ $service->service_id }}">
                                                        <svg class="w-4 h-4 text-gray-500 dark:text-gray-400" fill="none" stroke="currentColor"
                                                            viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                                d="M12 5v.01M12 12v.01M12 19v.01M12 6a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2z" />
                                                        </svg>
                                                    </button>

                                                    <div id="dropdown-menu-{{ $service->service_id }}"
                                                        class="hidden menu-button-action absolute right-0 z-50 mt-2 w-48 bg-white dark:bg-gray-800 rounded-md shadow-lg border border-gray-200 dark:border-gray-700 ring-1 ring-black ring-opacity-5 focus:outline-none">
                                                        <div class="py-1 z-500" role="menu">
                                                            <button onclick="toggleDetails({{ $service->service_id }}); closeDropdown({{ $service->service_id }})"
                                                                class="flex items-center w-full px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors duration-150"
                                                                role="menuitem">
                                                                Xem chi tiết
                                                            </button>
                                                            <a href="{{ route('admin.services.services.edit', $service->service_id) }}"
                                                                class="flex items-center w-full px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors duration-150"
                                                                role="menuitem">
                                                                Chỉnh sửa
                                                            </a>
                                                            <div class="border-t border-gray-100 dark:border-gray-700 my-1"></div>
                                                            <button onclick="deleteService({{ $service->service_id }}); closeDropdown({{ $service->service_id }})"
                                                                class="flex items-center w-full px-4 py-2 cursor-pointer text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors duration-150"
                                                                role="menuitem">
                                                                <svg style="width: 20px;" class="mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                                        d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                                                </svg>
                                                                Xoá dịch vụ
                                                            </button>
                                                        </div>
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
                                        <!-- Expandable Details Row -->
                                        <tr id="details-{{ $service->service_id }}" class="hidden bg-gray-50 dark:bg-gray-700/50">
                                            <td colspan="7" class="px-5 py-4">
                                                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                                    <!-- Left Column -->
                                                    <div class="space-y-4">
                                                        <!-- Tên & Mô tả -->
                                                        <div>
                                                            <h4 class="font-semibold text-gray-800 dark:text-gray-100 mb-2">Tên dịch vụ:</h4>
                                                            <p class="text-sm text-gray-700 dark:text-gray-300">{{ $service->name }}</p>
                                                        </div>
                                                        <div>
                                                            <h4 class="font-semibold text-gray-800 dark:text-gray-100 mb-2">Mô tả:</h4>
                                                            <p class="text-sm text-gray-600 dark:text-gray-400">
                                                                {{ $service->description ?? 'Chưa có mô tả' }}
                                                            </p>
                                                        </div>
                                                        <div>
                                                            <h4 class="font-semibold text-gray-800 dark:text-gray-100 mb-2">Đơn vị:</h4>
                                                            <p class="text-sm text-gray-700 dark:text-gray-300">{{ $service->unit ?? 'Không có' }}</p>
                                                        </div>
                                                    </div>

                                                    <!-- Right Column -->
                                                    <div class="space-y-4">
                                                        <div>
                                                            <h4 class="font-semibold text-gray-800 dark:text-gray-100 mb-2">Trạng thái:</h4>
                                                            <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium 
                                                                {{ $service->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                                                {{ $service->is_active ? 'Hoạt động' : 'Không hoạt động' }}
                                                            </span>
                                                        </div>
                                                        <div>
                                                            <h4 class="font-semibold text-gray-800 dark:text-gray-100 mb-2">Timestamps:</h4>
                                                            <div class="text-sm text-gray-600 dark:text-gray-400 space-y-1">
                                                                <div>Created: {{ $service->created_at->format('d/m/Y H:i') }}</div>
                                                                <div>Updated: {{ $service->updated_at->format('d/m/Y H:i') }}</div>
                                                            </div>
                                                        </div>
                                                        <!-- Close Button -->
                                                        <div>
                                                            <button onclick="toggleDetails({{ $service->service_id }})"
                                                                class="btn cursor-pointer bg-indigo-500 hover:bg-indigo-600 text-white">
                                                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                                                                </svg>
                                                                Đóng
                                                            </button>

                                                        </div>
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="7" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">Chưa có dịch vụ nào được tạo.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
                        </div>

                        <!-- Pagination -->
                        <div class="mt-4">
                            {{ $services->links() }}
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>


    <script>
        // Animation khi hiển thị
        document.getElementById('notification').classList.add('translate-y-0', 'opacity-100');
        document.getElementById('notification').classList.remove('-translate-y-full', 'opacity-0');

        // Tự động ẩn sau 5 giây
        setTimeout(() => {
            closeNotification();
        }, 5000);

        function closeNotification() {
            const notification = document.getElementById('notification');
            notification.classList.add('opacity-0', 'scale-95');
            setTimeout(() => {
                notification.remove();
            }, 300);
        }
    </script>
    <script>
        function toggleDetails(serviceId) {
            const detailsRow = document.getElementById(`details-${serviceId}`);
            const isCurrentlyVisible = !detailsRow.classList.contains('hidden');

            // Ẩn toàn bộ trước
            document.querySelectorAll('[id^="details-"]').forEach(el => el.classList.add('hidden'));

            // Nếu đang ẩn thì hiển thị lại, nếu đang hiển thị thì ẩn đi (đóng)
            if (!isCurrentlyVisible) {
                detailsRow.classList.remove('hidden');
            }
        }


        function toggleDropdown(serviceId) {
            document.querySelectorAll('[id^="dropdown-menu-"]').forEach(el => el.classList.add('hidden'));
            document.getElementById(`dropdown-menu-${serviceId}`).classList.toggle('hidden');
        }

        function closeDropdown(serviceId) {
            document.getElementById(`dropdown-menu-${serviceId}`).classList.add('hidden');
        }

        document.addEventListener('click', function(event) {
            if (event.target.closest('.button-action') || event.target.closest('[id^="dropdown-menu-"]')) {
                return;
            }
            document.querySelectorAll('[id^="dropdown-menu-"]').forEach(el => el.classList.add('hidden'));
        });

        function toggleStatus(serviceId) {
            if (confirm('Bạn có chắc muốn đổi trạng thái?')) {
                fetch(`/admin/services/services/toggle-status/${serviceId}`, {
                    method: 'PATCH',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                        'Content-Type': 'application/json',
                    },
                })
                .then(response => response.json())
                .then(() => location.reload())
                .catch(() => alert('Có lỗi xảy ra!'));
            }
        }

        function deleteService(serviceId) {
            if (confirm('Xác nhận xoá dịch vụ này?')) {
                const form = document.createElement('form');
                form.method = 'POST';
                form.action = `/admin/services/services/destroy/${serviceId}`;
                form.innerHTML = `<input type="hidden" name="_token" value="${document.querySelector('meta[name="csrf-token"]').content}">`;
                document.body.appendChild(form);
                form.submit();
            }
        }
    </script>

    <style>
        .fade-out {
            animation: fadeOut 0.5s ease-out forwards;
        }

        @keyframes fadeOut {
            from { opacity: 1; transform: translateY(0); }
            to { opacity: 0; transform: translateY(-20px); }
        }
    </style>
</x-app-layout>
