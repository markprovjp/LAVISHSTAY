<style>
    ...
    .menu-button-action {
        position: fixed;
        /* Sử dụng fixed để menu không bị giới hạn bởi container cha */
        min-width: 200px;
        /* Đảm bảo chiều rộng tối thiểu */
        background-color: #fff;
        /* Đảm bảo nền trắng cho chế độ sáng */
    ...
    .dark .menu-button-action {
        background-color: #1f2937;
        /* Nền tối cho dark mode */
        border-color: #374151;
        /* Border cho dark mode */
    }
...
<x-app-layout>
            ...
            <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
                <a href="{{ route('admin.users.customers.create') }}">
                    <button
                        ...
                        <span class="max-xs:sr-only">Thêm Khách hàng</span>
                    </button>
                </a>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 items-end">
                ...
                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">
                        <i class="fas fa-user mr-1 text-violet-600"></i> Họ và tên
                    </label>
                ...
                <div>
                    <label for="keyword" class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">
                        <i class="fas fa-at mr-1 text-violet-600"></i> Email hoặc Số điện thoại
                    </label>
                ...
                <div>
                    <label for="identity_code" class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">
                        <i class="fas fa-id-card mr-1 text-violet-600"></i> Số CCCD/Hộ chiếu
                    </label>
            ...
            <div class="p-6 text-gray-900 dark:text-gray-100">
                @if ($users->count() > 0)
                    ...
                    @if ($users->hasPages())
                        <div class="mt-4">
                            @php
                                \Illuminate\Pagination\Paginator::defaultView('vendor.pagination.tailwind'); // Hoặc 'bootstrap-4' nếu dùng Bootstrap
                                \Illuminate\Pagination\Paginator::defaultSimpleView(
                                    'vendor.pagination.simple-tailwind',
                                );
                                app('translator')->set([
                                    'pagination.previous' => '&laquo; Trước',
                                    'pagination.next' => 'Tiếp &raquo;',
                                    'pagination.showing' => 'Hiển thị từ :from đến :to của :total kết quả',
                                ]);
                            @endphp
                            {!! $users->links() !!}
                        </div>
                ...
                @else
                    ...
                    <div class="text-center py-16">
                        ...
                        <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">Không tìm thấy khách hàng
                        </h3>
                        <p class="text-gray-500 dark:text-gray-400 mb-6">Bắt đầu bằng cách tạo khách hàng đầu tiên
                            trong
                            hệ thống.</p>
                        <a href="{{ route('admin.users.customers.create') }}"
    ...
    <script>
        ...
        function toggleDropdown(userId) {
            ...
            const button = document.getElementById(`dropdown-button-${userId}`);
            ...
            // Nếu dropdown đang hiển thị, tính toán vị trí
            if (!dropdown.classList.contains('hidden')) {
                const buttonRect = button.getBoundingClientRect();
                const dropdownRect = dropdown.getBoundingClientRect();
                const viewportHeight = window.innerHeight;

                // Định vị dropdown bên trái nút trên desktop
                let leftPosition = buttonRect.left - dropdownRect.width - 8; // 8px khoảng cách
                let topPosition = buttonRect.top + (buttonRect.height / 2) - (dropdownRect.height / 2) +
                45; // Căn giữa theo chiều dọc

                // Nếu là mobile (dưới 640px), hiển thị dropdown bên dưới nút
                if (window.innerWidth <= 640) {
                    leftPosition = buttonRect.left;
                    topPosition = buttonRect.bottom + 5; // 5px khoảng cách dưới nút
                }

                // Đảm bảo dropdown không bị tràn ra ngoài viewport
                if (leftPosition < 0) {
                    leftPosition = buttonRect.right + 8; // Hiển thị bên phải nếu không đủ chỗ bên trái
                }
                if (topPosition + dropdownRect.height > viewportHeight) {
                    topPosition = viewportHeight - dropdownRect.height - 10; // Đảm bảo không tràn xuống dưới
                }
                if (topPosition < 0) {
                    topPosition = 10; // Đảm bảo không tràn lên trên
                }
