...
<div class="relative inline-flex" x-data="{ open: false }">
    ...
    >
        ...
        <ul>
            ...
            @if (Auth::user()?->hasPermission('thong_bao'))
                <li>
                    <a class="font-medium text-sm text-violet-500 hover:text-violet-600 dark:hover:text-violet-400 flex items-center py-1 px-3" href="{{ route('admin.notifications.index', Auth::user()->role_id) }}" @click="open = false" @focus="open = true" @focusout="open = false">Thông báo</a>
                </li>
            @endif
