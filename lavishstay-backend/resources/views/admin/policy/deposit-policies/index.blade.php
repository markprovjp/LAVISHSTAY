    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-5">
            ...
            <div class="mb-4 sm:mb-0">
                <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold">Chính sách đặt cọc</h1>
                <p class="text-sm text-gray-600 dark:text-gray-400">Quản lý các chính sách đặt cọc cho đặt phòng</p>
            </div>

            <!-- Right: Actions -->
            <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
                <!-- Add policy button -->
                <a href="{{ route('admin.deposit-policies.create') }}"
        ...
        <!-- Filters -->
        <div class="py-5">
            <form method="GET" action="{{ route('admin.deposit-policies') }}" class="flex flex-wrap gap-4">
                <!-- Search -->
                ...
                <div>
                    <select name="sort_by" class=" border border-gray-300 dark:border-gray-600 rounded-md shadow-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500">
                        ...
                        <option value="deposit_percentage"
                            {{ request('sort_by') === 'deposit_percentage' ? 'selected' : '' }}>Phần trăm</option>
                        <option value="deposit_fixed_amount_vnd"
                            {{ request('sort_by') === 'deposit_fixed_amount_vnd' ? 'selected' : '' }}>Số tiền</option>
                    </select>
                </div>
                ...
                <table class="table-auto w-full dark:text-gray-300">
                    ...
                    <thead
                        ...
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left">Phần trăm đặt cọc</th>
                            <th class="px-6 py-4 text-left">Số tiền cố định</th>
                    ...
                    <tbody class="text-sm divide-y divide-gray-100 dark:divide-gray-700/60">
                        @forelse($policies as $policy)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                    {{ $policy->deposit_percentage ? $policy->deposit_percentage . '%' : '-' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                    {{ $policy->deposit_fixed_amount_vnd ? number_format($policy->deposit_fixed_amount_vnd, 0, ',', '.') . ' VND' : '-' }}
                                </td>
                                    ...
                                    <div class="relative inline-block text-left">
                                        ...
                                        <div id="dropdown-menu-{{ $policy->policy_id }}"
                                            ...
                                            <div class="py-1 z-500" role="menu">
                                                ...
                                                <!-- Edit -->
                                                <a href="{{ route('admin.deposit-policies.edit', $policy->policy_id) }}"
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                        ...
                                        <div>
                                            <h4 class="font-medium text-gray-800 dark:text-gray-100 mb-2">Thông tin đặt
                                                cọc</h4>
                                            <div class="space-y-2">
                                                @if ($policy->deposit_percentage)
                                                    <div>
                                                        <span
                                                            class="text-xs font-medium text-gray-500 dark:text-gray-400">Phần
                                                            trăm:</span>
                                                        <div class="text-sm text-gray-900 dark:text-gray-100">
                                                            {{ $policy->deposit_percentage }}%</div>
                                                    </div>
                                                @endif
                                                @if ($policy->deposit_fixed_amount_vnd)
                                                    <div>
                                                        <span
                                                            class="text-xs font-medium text-gray-500 dark:text-gray-400">Số
                                                            tiền cố định:</span>
                                                        <div class="text-sm text-gray-900 dark:text-gray-100">
                                                            {{ number_format($policy->deposit_fixed_amount_vnd, 0, ',', '.') }}
                                                            VND</div>
                                                    </div>
                                                @endif
                                            </div>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">
                                    Chưa có chính sách đặt cọc nào được tạo.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <script>
        ...
        function toggleStatus(policyId) {
            if (confirm('Bạn có chắc chắn muốn thay đổi trạng thái của chính sách này?')) {
                fetch(`/admin/deposit-policies/${policyId}/toggle-status`, {
                        method: 'PATCH',
                        headers: {
        ...
        function deletePolicy(policyId) {
            if (confirm('Bạn có chắc chắn muốn xóa chính sách này? Hành động này không thể hoàn tác.')) {
                ...
                form.method = 'POST';
                form.action = `/admin/deposit-policies/${policyId}`;
