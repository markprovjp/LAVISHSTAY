    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            <div class="mb-4 sm:mb-0">
                <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold">Chỉnh Sửa Chính Sách Bồi Thường</h1>
                <p class="text-sm text-gray-600 dark:text-gray-400">Cập nhật thông tin cho chính sách bồi thường</p>
            </div>
        ...
        <x-slot name="header">
            <div class="flex justify-between items-center">
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                    {{ __('Chỉnh Sửa Chính Sách Bồi Thường') }}
                </h2>
            </div>
        </x-slot>
            ...
            <div class="">
                <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        ...
                        <form method="POST" action="{{ route('admin.compensation-policies.update', $policy->compensation_policy_id) }}" class="space-y-6">
                            @csrf
                            @method('PUT')
                            <div>
                                ...
                                <input type="text" name="name" id="name" value="{{ old('name', $policy->name) }}" class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-violet-500 focus:border-violet-500 dark:bg-gray-700 dark:text-gray-100" required>
                            </div>
                            <div>
                                <label for="description" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Mô tả</label>
                                <textarea name="description" id="description" class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-violet-500 focus:border-violet-500 dark:bg-gray-700 dark:text-gray-100">{{ old('description', $policy->description) }}</textarea>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <input type="number" name="discount_value" id="discount_value" value="{{ old('discount_value', $policy->discount_value) }}" step="0.01" min="0" class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-violet-500 focus:border-violet-500 dark:bg-gray-700 dark:text-gray-100" required>
                            </div>
                            <div>
                                ...
                                <input type="number" name="max_compensation_amount" id="max_compensation_amount" value="{{ old('max_compensation_amount', $policy->max_compensation_amount) }}" step="0.01" min="0" class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-violet-500 focus:border-violet-500 dark:bg-gray-700 dark:text-gray-100">
                            </div>
                            <div>
                                <label for="is_active" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Trạng thái</label>
                                <input type="checkbox" name="is_active" id="is_active" {{ old('is_active', $policy->is_active) ? 'checked' : '' }} class="mt-1 h-4 w-4 text-violet-600 focus:ring-violet-500 border-gray-300 dark:border-gray-600 rounded">
