    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-5">
            ...
            <div class="mb-4 sm:mb-0">
                <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold">Chính sách chuyển phòng</h1>
                <p class="text-sm text-gray-600 dark:text-gray-400">Quản lý các chính sách chuyển phòng cho đặt phòng</p>
            </div>

            <!-- Right: Actions -->
            <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
                <!-- Add policy button -->
                <a href="{{ route('admin.transfer-policies.create') }}"
        ...
        <!-- Filters -->
        <div class="py-5">
            <form method="GET" action="{{ route('admin.transfer-policies') }}" class="flex flex-wrap gap-4">
                <!-- Search -->
                ...
                <div>
                    <select name="sort_by" class=" border border-gray-300 dark:border-gray-600 rounded-md shadow-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500">
                        ...
                        <option value="transfer_fee_vnd"
                            {{ request('sort_by') === 'transfer_fee_vnd' ? 'selected' : '' }}>Phí chuyển phòng</option>
                        <option value="min_days_before_check_in"
                            {{ request('sort_by') === 'min_days_before_check_in' ? 'selected' : '' }}>Ngày tối thiểu trước check-in</option>
                    </select>
                </div>
                    ...
                    <tbody class="text-sm divide-y divide-gray-100 dark:divide-gray-700/60">
                        @forelse($policies as $policy)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                    {{ $policy->transfer_fee_vnd ? number_format($policy->transfer_fee_vnd, 0, ',', '.') . ' VND' : '-' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                    {{ $policy->transfer_fee_percentage ? $policy->transfer_fee_percentage . '%' : '-' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                    {{ $policy->min_days_before_check_in ?? '-' }} ngày
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $policy->requires_guest_confirmation ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800' }}">
                                        {{ $policy->requires_guest_confirmation ? 'Yêu cầu' : 'Không yêu cầu' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <div class="relative inline-block text-left">
                                        ...
                                        <div id="dropdown-menu-{{ $policy->policy_id }}"
                                            ...
                                            <div class="py-1 z-500" role="menu">
                                                ...
                                                <!-- Edit -->
                                                <a href="{{ route('admin.transfer-policies.edit', $policy->policy_id) }}"
                            ...
                            <tr id="details-{{ $policy->policy_id }}" class="hidden bg-gray-50 dark:bg-gray-900/50">
                                <td colspan="10" class="px-6 py-4">
                                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                        ...
                                        <div>
                                            <h4 class="font-medium text-gray-800 dark:text-gray-100 mb-2">Thông tin chuyển phòng
                                            </h4>
                                            <div class="space-y-2">
                                                @if ($policy->transfer_fee_vnd)
                                                    <div>
                                                        <span
                                                            class="text-xs font-medium text-gray-500 dark:text-gray-400">Phí chuyển phòng:</span>
                                                        <div class="text-sm text-gray-900 dark:text-gray-100">
                                                            {{ number_format($policy->transfer_fee_vnd, 0, ',', '.') }}
                                                            VND</div>
                                                    </div>
                                                @endif
                                                @if ($policy->transfer_fee_percentage)
                                                    <div>
                                                        ...
                                                        <div class="text-sm text-gray-900 dark:text-gray-100">
                                                            {{ $policy->transfer_fee_percentage }}%</div>
                                                    </div>
                                                @endif
                                                @if ($policy->min_days_before_check_in)
                                                    <div>
                                                        <span
                                                            class="text-xs font-medium text-gray-500 dark:text-gray-400">Ngày tối thiểu trước check-in:</span>
                                                        <div class="text-sm text-gray-900 dark:text-gray-100">
                                                            {{ $policy->min_days_before_check_in }} ngày</div>
                                                    </div>
                                                @endif
                                                <div>
                                                    <span
                                                        class="text-xs font-medium text-gray-500 dark:text-gray-400">Yêu cầu xác nhận khách:</span>
                                                    <div class="flex flex-wrap gap-1 mt-1">
                                                        <span
                                                            class="inline-flex items-center font-normal py-1 px-2 rounded-full text-xs {{ $policy->requires_guest_confirmation ? 'bg-yellow-100 dark:bg-yellow-400/30 text-yellow-600 dark:text-yellow-400' : 'bg-gray-100 dark:bg-gray-400/30 text-gray-600 dark:text-gray-400' }}">
                                                            {{ $policy->requires_guest_confirmation ? 'Có' : 'Không' }}
                                                        </span>
                                                    </div>
                                                </div>
                                            </div>
                        ...
                        @empty
                            <tr>
                                <td colspan="10" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">
                                    Chưa có chính sách chuyển phòng nào được tạo.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <script>
        ...
        function toggleStatus(policyId) {
            if (confirm('Bạn có chắc chắn muốn thay đổi trạng thái của chính sách này?')) {
                fetch(`/admin/transfer-policies/${policyId}/toggle-status`, {
                        method: 'PATCH',
                        headers: {
        ...
        function deletePolicy(policyId) {
            if (confirm('Bạn có chắc chắn muốn xóa chính sách này? Hành động này không thể hoàn tác.')) {
                ...
                form.method = 'POST';
                form.action = `/admin/transfer-policies/${policyId}`;
