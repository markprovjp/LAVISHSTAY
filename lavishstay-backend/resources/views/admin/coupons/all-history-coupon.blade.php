<x-app-layout>
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            ...
            <div class="mb-4 sm:mb-0">
                <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold">Tổng quan lịch sử Coupon</h1>
                <p class="text-sm text-gray-600 dark:text-gray-400">Xem tất cả lịch sử sử dụng coupon trong hệ thống</p>
            </div>

            <!-- Right: Actions -->
            <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
                <button onclick="exportData()" class="btn bg-green-600 text-white hover:bg-green-700">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mb-8">
            ...
            <div class="bg-gradient-to-r from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 rounded-xl p-6 border border-blue-200 dark:border-blue-800">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <div class="text-2xl font-bold text-blue-900 dark:text-blue-100">{{ $stats['total_redemptions'] ?? 0 }}</div>
            ...
            <div class="bg-gradient-to-r from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/20 rounded-xl p-6 border border-purple-200 dark:border-purple-800">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <div class="text-2xl font-bold text-purple-900 dark:text-purple-100">{{ $stats['active_coupons'] ?? 0 }}</div>
                        <div class="text-sm text-purple-700 dark:text-purple-300">Coupon đang hoạt động</div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-r from-orange-50 to-orange-100 dark:from-orange-900/20 dark:to-orange-800/20 rounded-xl p-6 border border-orange-200 dark:border-orange-800">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <div class="text-2xl font-bold text-orange-900 dark:text-orange-100">{{ $stats['unique_users'] ?? 0 }}</div>
                        <div class="text-sm text-orange-700 dark:text-orange-300">Người dùng đã sử dụng</div>
                    </div>
                </div>
            </div>
        </div>
                ...
                <form method="GET" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-4 items-end">
                    <div>
                        ...
                        <input type="text" name="search" value="{{ request('search') }}"
                            placeholder="Mã coupon, tên khách hàng..."
                            class="block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm w-full placeholder-gray-400 dark:placeholder-gray-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Coupon</label>
                        <select name="coupon_id" class="border border-gray-300 dark:border-gray-600 rounded-md shadow-sm w-full bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500">
                            <option value="">Tất cả coupon</option>
                    ...
                    <div>
                        ...
                        <input type="date" name="date_from" value="{{ request('date_from') }}"
                            class="border border-gray-300 dark:border-gray-600 rounded-md shadow-sm w-full bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500">
                    </div>
                    ...
                    <div class="flex gap-2">
                        <button type="submit" class="btn bg-violet-600 text-white hover:bg-violet-700 flex-1">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @if(request()->hasAny(['search', 'coupon_id', 'date_from', 'date_to']))
                            <a href="{{ route('admin.coupons.all-redemptions') }}" class="btn bg-gray-500 text-white hover:bg-gray-600">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                    ...
                                    @if($redemption->coupon)
                                        <div>
                                            ...
                                            <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                                @if($redemption->coupon->type === 'percent')
                                                    {{ $redemption->coupon->value }}%
                                                @else
                                                    {{ number_format($redemption->coupon->value, 0, ',', '.') }} VND
                                                @endif
                                            </div>
                                        </div>
                                    @else
                                        <span class="text-gray-500 dark:text-gray-400">Coupon đã xóa</span>
                                    @endif
                                </td>
                                    ...
                                    @if($redemption->user)
                                        <div class="flex items-center">
                                            <div class="flex-shrink-0 h-8 w-8">
                                                <div class="h-8 w-8 rounded-full bg-gradient-to-r from-violet-400 to-purple-500 flex items-center justify-center shadow-sm">
                                            ...
                                            <div class="ml-3">
                                                <div class="font-medium text-gray-900 dark:text-gray-100">{{ Str::limit($redemption->user->name, 20) }}</div>
                                                <div class="text-xs text-gray-500 dark:text-gray-400">{{ Str::limit($redemption->user->email, 25) }}</div>
                                            </div>
                                        </div>
                                    @else
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="flex items-center justify-center space-x-2">
                                        @if($redemption->coupon)
                                            <a href="{{ route('admin.coupons.redemptions', $redemption->coupon->id) }}"
                                                class="inline-flex items-center px-2 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-xs font-medium rounded hover:bg-blue-200 dark:hover:bg-blue-900/50 transition-colors"
                                                title="Xem lịch sử coupon">
                                        ...
                                        @if($redemption->booking)
                                            <a href="#" onclick="viewBookingDetails({{ $redemption->booking_id }})"
                                                class="inline-flex items-center px-2 py-1 bg-violet-100 dark:bg-violet-900/30 text-violet-700 dark:text-violet-300 text-xs font-medium rounded hover:bg-violet-200 dark:hover:bg-violet-900/50 transition-colors"
                                                title="Xem booking">
                                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <tr>
                                <td colspan="8" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-sm text-gray-500 dark:text-gray-400">Chưa có coupon nào được sử dụng trong khoảng thời gian này.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
    ...
    <script>
        ...
        function exportData() {
            ...
            window.location.href = '{{ route("admin.coupons.all-redemptions") }}?' + params.toString();
        }
    </script>
</x-app-layout>
