...
[id^="dropdown-menu-"], [id^="language-dropdown-"] {
...
@media (max-width: 768px) {
    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full mx-auto">
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            <div class="mb-4 sm:mb-0">
                <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold">Chi tiết Bảng:
                    {{ $table }}</h1>
                                        ...
                                        <tr>
                                            ...
                                            @foreach ($languages as $lang)
                                                <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-100"
                                                    data-translation-id="{{ $recordTranslations->firstWhere('language_code', $lang->language_code)?->translation_id }}"
                                                    data-value="{{ $recordTranslations->firstWhere('language_code', $lang->language_code)?->value ?? 'Chưa dịch' }}">
                                            ...
                                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap text-center">
                                                <div class="relative inline-block text-left">
                                                    <button type="button"
                                                        ...
                                                        onclick="toggleDropdown('{{ $recordTranslations->first()->translation_id }}')"
                                                        id="dropdown-button-{{ $recordTranslations->first()->translation_id }}">
                                                    ...
                                                    <div id="dropdown-menu-{{ $recordTranslations->first()->translation_id }}"
                                                        class="hidden menu-button-action absolute right-0 z-50 mt-2 w-auto min-w-[160px] max-w-[200px] bg-white dark:bg-gray-800 rounded-md shadow-lg border border-gray-200 dark:border-gray-700 ring-1 ring-black ring-opacity-5 focus:outline-none">
                                                        <div class="py-1" role="menu">
                                                            <button
                                                                onclick="openEditModal('{{ $recordTranslations->first()->translation_id }}', '{{ $recordTranslations->first()->table_name }}', '{{ $recordTranslations->first()->column_name }}', '{{ $recordId }}', '{{ $recordTranslations->first()->language_code }}', '{{ $recordTranslations->first()->value ?? '' }}')"
                                                                ...
                                                                role="menuitem">Sửa bản dịch</button>
                                                            <div class="relative">
                                                                <button
                                                                    onclick="toggleLanguageDropdown('{{ $recordTranslations->first()->translation_id }}')"
                                                                ...
                                                                <div id="language-dropdown-{{ $recordTranslations->first()->translation_id }}"
                                                                    class="hidden absolute left-0 top-full mt-1 w-auto min-w-[160px] max-w-[200px] bg-white dark:bg-gray-800 rounded-md shadow-lg border border-gray-200 dark:border-gray-700 ring-1 ring-black ring-opacity-5">
                                                                    @foreach ($languages as $lang)
                                                                        <button
                                                                            onclick="deleteTranslation('{{ $recordTranslations->first()->translation_id }}', '{{ $recordId }}', '{{ $lang->language_code }}')"
                                                                            class="flex items-center w-full px-4 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors duration-150"
                                                                            role="menuitem">
                                                                            Xóa {{ $lang->name }}
                                                                        </button>
                                                            ...
                                                            <button
                                                                onclick="deleteRecordTranslations('{{ $table }}', '{{ $recordId }}')"
                                                                class="flex mt-2 items-center w-full px-4 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors duration-150"
                                                                role="menuitem">
                                                                <svg style="width: 20px; align-items: center"
        ...
        <!-- Modal Thêm Bản dịch -->
        <div id="add-translation-modal"
            class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50 overflow-auto">
            <div class="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-sm min-h-[300px]">
                ...
                <form id="add-translation-form" action="{{ route('admin.translation.add-for-table', $table) }}"
                    method="POST">
                    @csrf
 ...
 placeholder-gray-400 dark:placeholder-gray-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500">
                    ...
                    <div class="flex justify-end gap-4 mt-6">
                        <button type="button" onclick="closeAddModal()"
                            class="px-6 py-3 bg-red-400 text-white rounded-lg font-semibold shadow-md hover:bg-red-500 focus:outline-none focus:ring-4 focus:ring-red-300 transition duration-200 min-w-[100px]">
                            Hủy
                        </button>
                        <button type="submit"
                            class="px-6 py-3 btn bg-violet-500 hover:bg-violet-600 text-white rounded-lg 
                ...
                <form id="edit-translation-form">
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <input type="text" id="edit-column-name" name="column_name"
                                class="border w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm bg-white dark:bg-gray-700 text-gray-900 
 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500"
                                readonly>
                        </div>
                        <div>
                            ...
                            <input type="text" id="edit-record-id" name="record_id"
                                class="border w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm bg-white dark:bg-gray-700 text-gray-900 
 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500"
                                readonly>
                        </div>
 ...
 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500">
        ...
        <script>
            ...
            function openEditModal(translationId, tableName, columnName, recordId, languageCode, value) {
                ...
                const fetchTranslationValue = function() {
                    ...
                    const url =
                        `/admin/translation/get-value/${encodeURIComponent(tableName)}/${encodeURIComponent(columnName)}/${recordId}/${selectedLanguage}`;
    ...
    if (dropdown) {
        ...
        // Fix positioning khi zoom
        if (!dropdown.classList.contains('hidden')) {
            dropdown.style.position = 'fixed';
            dropdown.style.zIndex = '99998';
            
            const button = document.getElementById(`dropdown-button-${translationId}`);
    ...
    if (languageDropdown) {
        ...
        if (!languageDropdown.classList.contains('hidden')) {
            ...
            const triggerButton = document.querySelector(`button[onclick="toggleLanguageDropdown('${translationId}')"]`);
            ...
            function deleteTranslation(translationId, recordId, languageCode) {
                if (confirm(
                        `Bạn có chắc chắn muốn xóa bản dịch ngôn ngữ ${languageCode} không? Hành động này không thể hoàn tác!`
                        )) {
                    ...
                    console.log('Requesting deletion:', {
                        ...
                        languageCode,
                        csrfToken,
                        url: `/admin/translation/destroy/${translationId}/language/${languageCode}`
                    });
            ...
            function showToast(message) {
                ...
                toast.className =
                    'fixed bottom-5 right-5 z-50 bg-green-500 text-white px-4 py-3 rounded-md shadow-lg transition-opacity duration-300 opacity-0';
