            ...
            <aside class="space-y-6">
                ...
                <div x-data="{ hasThumbnailError: {{ $errors->has('thumbnail') || $errors->has('thumbnail_id') ? 'true' : 'false' }} }" x-cloak>
                    ...
                    <img id="thumbnail_preview"
                        src="{{ old('thumbnail_id', $news->thumbnail_id) ? optional($mediaFiles->firstWhere('id', old('thumbnail_id', $news->thumbnail_id)))->filepath : (file_exists(public_path('images/placeholder.png')) ? asset('images/placeholder.png') : asset('images/fallback.png')) }}"
               ...
               Tải ảnh mới</button>
                    ...
                    <input type="hidden" id="thumbnail_id" name="thumbnail_id"
                        value="{{ old('thumbnail_id', $news->thumbnail_id) }}" x-on:change="hasThumbnailError = false">
                    @error('thumbnail')
                ...
                <!-- Publish date -->
                <div x-data="{ publishDate: '{{ old('publish_date', $news->published_at->format('Y-m-d\TH:i')) }}', hasError: {{ $errors->has('publish_date') ? 'true' : 'false' }} }">
