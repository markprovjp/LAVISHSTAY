    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 max-w-9xl mx-auto">
        ...
        @if (session('success'))
            <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 3000)"
                 x-show="show" x-transition:leave="transition ease-in-out duration-500"
                 x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
                 class="mb-6 bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-lg shadow-md" role="alert">
                <span class="block sm:inline">{{ session('success') }}</span>
            </div>
        @endif

        <!-- No Search Results Message -->
        @if ($news->isEmpty() && (request()->has('search_title') || request()->has('category_id') || request()->has('author_id') || request()->has('status') || request()->has('search_date')))
            <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 3000)"
                 x-show="show" x-transition:leave="transition ease-in-out duration-500"
                 x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
                 class="mb-6 bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded-lg shadow-md" role="alert">
                <span class="block sm:inline">Không tìm thấy bài viết nào phù hợp với tiêu chí.</span>
            </div>
        @endif

        <!-- Table or No Articles Message -->
        @if ($news->isNotEmpty())
