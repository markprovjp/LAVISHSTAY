            ...
            <aside class="space-y-6">
                ...
                <div x-data="{ categoryId: '{{ old('category_id') }}', hasError: {{ $errors->has('category_id') ? 'true' : 'false' }} }">
                    ...
                    @if ($categories->isNotEmpty())
                        <select name="category_id" id="category_id"
                            class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200 py-2.5 px-3 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200"
                            x-model="categoryId" x-on:change="hasError = false">
                            <option value="">-- Chọn danh mục --</option>
                            @foreach ($categories as $cat)
                                <option value="{{ $cat->id }}"
                    ...
                    @else
                        <!-- Nếu không có danh mục, hiển thị ô input và nút chuyển hướng -->
                        <div class="flex items-center space-x-3">
                            <input type="text" disabled
                                class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 py-2.5 px-3 cursor-not-allowed placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none"
                                placeholder="Chưa có danh mục nào">
                            <a href="{{ route('admin.news.categories.index') }}"
                                class="inline-flex items-center px-4 py-2.5 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-gray-800 transition duration-200">
                                Thêm danh mục
                            </a>
                        </div>
                    @endif
                    ...
                    @error('category_id')
                        <p class="text-xs text-red-500 mt-1.5" x-show="hasError">{{ $message }}</p>
                    @enderror
                </div>
