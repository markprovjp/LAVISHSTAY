            ...
            <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
                <a href="{{ route('admin.floors.create') }}">
                    <button
                        ...
                        <span class="max-xs:sr-only cursor-pointer">Thêm Tầng</span>
                    </button>
                </a>
            </div>
        </div>
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                @forelse($floors as $floor)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">
                                            {{ Str::limit($floor->floor_name, 50) }}</td>
                                        <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">
                                            {{ $floor->floor_type }}</td>
                                        <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap text-center">
                                            <div class="relative inline-block text-left">
                                                <button type="button"
                                                    ...
                                                    onclick="toggleDropdown({{ $floor->floor_id }})"
                                                    id="dropdown-button-{{ $floor->floor_id }}">
                                                ...
                                                <div id="dropdown-menu-{{ $floor->floor_id }}"
                                                    ...
                                                    <div class="py-1 z-500" role="menu">
                                                        <button
                                                            onclick="toggleDetails({{ $floor->floor_id }}); closeDropdown({{ $floor->floor_id }})"
                                                            ...
                                                            role="menuitem">Xem Chi Tiết</button>
                                                        <a href="{{ route('admin.floors.edit', $floor->floor_id) }}"
                                                            class="flex items-center w-full px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors duration-150"
                                                            role="menuitem">Sửa Tầng</a>
                                                        <div class="border-t border-gray-100 dark:border-gray-700">
                                                        </div>
                                                        <button
                                                            onclick="deleteFloor({{ $floor->floor_id }}); closeDropdown({{ $floor->floor_id }})"
                                                    ...
                                                    <div>
                                                        ...
                                                        <div class="space-y-2">
                                                            ...
                                                            @if ($floor->floor_number)
                                                                <div><span
                                                                        class="text-xs font-medium text-gray-500 dark:text-gray-400">Floor
                                                                        Number:</span>
                                                                    <p
                                                                        class="text-sm text-green-600 dark:text-green-400">
                                                                        {{ $floor->floor_number }}</p>
                                                                </div>
                                                            @endif
                                                            @if ($floor->floor_type)
                                                                <div><span
                                                                        class="text-xs font-medium text-gray-500 dark:text-gray-400">Type:</span>
                                                                    <p
                                                                        class="text-sm text-green-600 dark:text-green-400">
                                                                        {{ $floor->floor_type }}</p>
                                                                </div>
                                                            ...
                                                            @if ($floor->facilities)
                                                                <div><span
                                                                        class="text-xs font-medium text-gray-500 dark:text-gray-400">Facilities:</span>
                                                                    <p
                                                                        class="text-sm text-green-600 dark:text-green-400">
                                                                        {{ $floor->facilities }}</p>
                                                                </div>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5"
                                            class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">Chưa có tầng
                                            nào được tạo.</td>
                                    </tr>
                                @endforelse
                            </tbody>
    ...
    <script>
        ...
        function toggleDropdown(floorId) {
            ...
            allDropdowns.forEach(menu => {
                if (menu.id !== `dropdown-menu-${floorId}`) menu.classList.add('hidden');
            });
        ...
        document.addEventListener('click', function(event) {
            ...
            if (!clickedInsideDropdown) dropdowns.forEach(dropdown => {
                dropdown.classList.add('hidden');
            });
        });
            ...
            if (confirm('Bạn có chắc chắn muốn xóa tầng này? Hành động này không thể hoàn tác!')) {
                ...
                fetch(form.action, {
                    ...
                    .then(data => {
                        ...
                        else showNotification(data.error || 'Lỗi khi xóa tầng', 'error');
                    })
                    .catch(error => {
                        console.error('Delete failed:', error);
                        showNotification('Lỗi khi xóa tầng', 'error');
                    });
            }
        }
