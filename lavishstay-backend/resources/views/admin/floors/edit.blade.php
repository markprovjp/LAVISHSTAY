    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            <div class="mb-4 sm:mb-0">
                <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold">Chỉnh Sửa Tầng</h1>
                <p class="text-sm text-gray-600 dark:text-gray-400">Cập nhật thông tin tầng</p>
            </div>
            ...
            <div class="">
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        ...
                        @endif
                        <form action="{{ route('admin.floors.update', $floor->floor_id) }}" method="POST" id="floorForm">
                            @csrf
                            @method('PUT')
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <select name="floor_type" id="floor_type" class="form-input w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500 dark:bg-gray-700 dark:text-white" required>
                                        ...
                                        <option value="service" {{ old('floor_type', $floor->floor_type) === 'service' ? 'selected' : '' }}>Dịch vụ</option>
                                        <option value="special" {{ old('floor_type', $floor->floor_type) === 'special' ? 'selected' : '' }}>Đặc biệt</option>
                                    </select>
                                </div>
                                <div>
                                    ...
                                    <select name="is_active" id="is_active" class="form-input w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500 dark:bg-gray-700 dark:text-white" required>
                                        <option value="1" {{ old('is_active', $floor->is_active) === 1 ? 'selected' : '' }}>Hoạt động</option>
                                        <option value="0" {{ old('is_active', $floor->is_active) === 0 ? 'selected' : '' }}>Không hoạt động</option>
                                    </select>
                                </div>
                                <div class="md:col-span-2">
                                    <label for="description" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Mô Tả</label>
                                    <textarea name="description" id="description" rows="5" class="form-input w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500 dark:bg-gray-700 dark:text-white" placeholder="Mô tả tầng...">{{ old('description', $floor->description) }}</textarea>
                                </div>
                                <div class="md:col-span-2">
                                    <label for="facilities" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Tiện Ích</label>
                                    <textarea name="facilities" id="facilities" rows="5" class="form-input w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500 dark:bg-gray-700 dark:text-white" placeholder="Tiện ích tầng...">{{ old('facilities', $floor->facilities) }}</textarea>
                                </div>
                            </div>
                            <div class="px-6 py-4 border-t mt-5 rounded-b-lg">
                                <div class="flex items-center justify-end space-x-3">
                                    ...
                                    <button type="submit" class="btn cursor-pointer bg-violet-500 hover:bg-violet-600 text-white">Cập Nhật Tầng</button>
                                </div>
                            </div>
                        </form>
                    </div>
