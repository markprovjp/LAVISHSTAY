<x-app-layout>
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            ...
            <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
                ...
                <a href="{{ route('admin.pricing.calculator') }}" class="btn bg-green-500 hover:bg-green-600 text-white">
                    <svg class="fill-current shrink-0 xs:hidden" width="16" height="16" viewBox="0 0 16 16">
                        <path d="M8 0L3 5h3v6h4V5h3L8 0zM1 14h14v2H1v-2z" />
                    </svg>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-6">
                ...
                <a href="{{ route('admin.flexible-pricing') }}" class="btn bg-blue-500 hover:bg-blue-600 text-white w-full">
                    Quản lý quy tắc
                </a>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-6">
                ...
                <a href="{{ route('admin.dynamic-pricing.index') }}" class="btn bg-green-500 hover:bg-green-600 text-white w-full">
                    Quản lý quy tắc
                </a>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-6">
                ...
                <a href="{{ route('admin.pricing.history') }}" class="btn bg-purple-500 hover:bg-purple-600 text-white w-full">
                    Xem lịch sử
                </a>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-6">
            <div class="flex items-center justify-between mb-6">
                ...
                <div class="flex space-x-2">
                    <select id="roomTypeSelect" class="form-select text-sm">
                        <option value="">Chọn loại phòng</option>
                    </select>
                    <button id="refreshPreview" class="btn bg-gray-500 hover:bg-gray-600 text-white text-sm">
                        Làm mới
                    </button>
                </div>
            </div>
    ...
    <script>
        ...
        async function loadPricingPreview() {
            ...
            container.innerHTML = '<div class="text-center py-8"><div class="animate-spin rounded-full h-8 w-8 border-b-2 border-indigo-500 mx-auto"></div></div>';

            try {
                const startDate = new Date().toISOString().split('T')[0];
                const endDate = new Date(Date.now() + 6 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
                
                const response = await fetch(`{{ route("admin.pricing.preview") }}?room_type_id=${roomTypeId}&start_date=${startDate}&end_date=${endDate}`);
                const data = await response.json();
                ...
                html += `
                    <tr>
                        ...
                        <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">
                            ${item.applied_rules.length > 0 ? item.applied_rules.map(rule => rule.rule_type).join(', ') : 'Không có'}
                        </td>
                    </tr>
                `;
            });
