...
class BookingCancellationController extends Controller
{
    ...
    public function getBookingCancellationInfo(Request $request, $bookingId)
    {
        // Validate booking ID
        if (!is_numeric($bookingId)) {
            return response()->json(['error' => 'ID đặt phòng không hợp lệ'], 400);
        ...
        // Log policy for debugging
        Log::info('Cancellation Policy Preview:', [
            'policy' => $cancellationPolicy ? $cancellationPolicy->toArray() : null,
            'time_diff' => $timeDiff,
            'check_in_date' => $booking->check_in_date->format('Y-m-d H:i:s'),
            'cancel_time' => $cancelTime->format('Y-m-d H:i:s'),
        ...
        // Log relations for debugging
        Log::info('Booking Relations Preview:', [
            'booking_id' => $booking->booking_id,
            'booking_rooms' => $bookingRooms->toArray(),
            'room_names' => $roomNames,
            'room_type' => $roomType ? $roomType->toArray() : null,
        ...
        // Get policy name and ID safely
        $policyName = $cancellationPolicy->name ?? 'Chính sách mặc định';
        $policyId = $cancellationPolicy->policy_id ?? null;

        // Check if cancellation would be free
        if ($timeDiff >= ($cancellationPolicy->free_cancellation_days ?? PHP_INT_MAX)) {
            return response()->json([
                'message' => 'Đặt phòng có thể được hủy miễn phí',
                'reason' => "Nếu hủy vào ngày {$cancelTime->format('Y-m-d')}, cách ngày nhận phòng ({$booking->check_in_date->format('Y-m-d')} 14:00) " . round($timeDiff, 2) . " ngày, sẽ thỏa mãn chính sách hủy miễn phí {$policyName} (trước {$cancellationPolicy->free_cancellation_days} ngày).",
                'formula' => 'Không áp dụng',
                'policy' => $policyName,
                'policy_id' => $policyId,
                'penalty' => 0,
