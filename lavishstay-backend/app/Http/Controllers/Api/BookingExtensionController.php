...
class BookingExtensionController extends Controller
{
    ...
    public function getExtendBookingInfo(Request $request, $bookingId)
    {
        try {
            Log::info('=== BookingController@getExtendBookingInfo START ===');
            Log::info('Request URL: ' . $request->fullUrl());
            Log::info('Request data: ', $request->all());
            Log::info('Query parameters: ', $request->query());

            // Validate input (accept new_check_out_date from query or body)
            $newCheckOutDateInput = $request->query('new_check_out_date') ?? $request->input('new_check_out_date');
            if (!$newCheckOutDateInput) {
                Log::error('Missing new_check_out_date');
                return response()->json(['error' => 'Ngày trả phòng mới là bắt buộc'], 422);
            }

            try {
                $newCheckOutDate = Carbon::parse($newCheckOutDateInput)->startOfDay();
                if (!$newCheckOutDate->isValid()) {
                    throw new \Exception('Invalid date format');
                }
            } catch (\Exception $e) {
                Log::error('Invalid new_check_out_date format', ['value' => $newCheckOutDateInput]);
                return response()->json(['error' => 'Định dạng ngày trả phòng mới không hợp lệ'], 422);
            }
            ...
            $bookingInfo = [
                ...
                'check_in_date' => Carbon::parse($booking->check_in_date)->toDateTimeString(),
                'check_out_date' => $newCheckOutDate->toDateTimeString(),
                'total_price' => $booking->total_price_vnd + $extensionFee,
            ];
            ...
            Log::info('=== BookingController@getExtendBookingInfo SUCCESS ===');

            return response()->json([
                'success' => true,
                'message' => 'Thông tin gia hạn đặt phòng',
                'reason' => "Thông tin dự kiến cho yêu cầu gia hạn vào ngày {$currentTime->format('Y-m-d')}, cách ngày trả phòng hiện tại ({$currentCheckOutDate->format('Y-m-d')}) {$timeDiff} ngày, thỏa mãn chính sách gia hạn {$policyName}. Số ngày gia hạn: {$extensionDays}, phí gia hạn: {$extensionFee} VND.",
                'formula' => $extensionPolicy->extension_percentage > 0
                    ? 'Phí gia hạn = (Phí cố định + (Giá phòng/đêm * Tỷ lệ phần trăm)) * Số ngày'
                    : 'Phí gia hạn = Phí cố định * Số ngày',
                'policy' => $policyName,
            ...
            Log::error('=== ERROR in getExtendBookingInfo ===');
            Log::error('Error message: ' . $e->getMessage());
            Log::error('File: ' . $e->getFile());
            Log::error('Line: ' . $e->getLine());
            Log::error('Stack trace: ' . $e->getTraceAsString());
            ...
            return response()->json([
                'success' => false,
                'message' => 'Có lỗi xảy ra khi lấy thông tin gia hạn',
                'error' => $e->getMessage(),
                'line' => $e->getLine(),
                'file' => basename($e->getFile())
            ], 500);
