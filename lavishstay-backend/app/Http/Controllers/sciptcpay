  function doGet(e) {
    try {
      ...
      Logger.log('🔍 CPay API Request: ' + JSON.stringify({
        action: action,
  ...
  function doPost(e) {
    try {
      ...
      Logger.log('🔍 CPay API POST Request: ' + JSON.stringify({
        action: action,
        booking_code: bookingCode,
        amount: amount
      }));
  ...
  function checkPayment(bookingCode, expectedAmount) {
    try {
      Logger.log(`🔍 Checking payment for booking: ${bookingCode}, amount: ${expectedAmount}`);
      
      // Open the Google Sheet (thay bằng ID sheet của bạn)
      const SHEET_ID = '1NcE0gzaNR9rpXqxJG-YAoiTBjyawJwPaM_EF363GwAY'; // Thay bằng ID thực của sheet
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        Logger.log(`🔍 Checking row ${i + 1}: "${description}" for pattern "${searchPattern}"`);
        ...
        if (description.includes(searchPattern) && type === 'Tiền vào') {
          ...
          Logger.log(`💰 Amount check: ${amount} vs ${expectedAmount}, match: ${amountMatch}`);
